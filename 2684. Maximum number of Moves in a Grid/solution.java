...
*/
    ...
    public int solveCurMoves(int[][] grid, int row, int col, int curNum){
        curStep++;
        if(curStep > maxStep)
            maxStep = curStep;
        ...
        if( (row != 0) && grid[row - 1][col + 1] > curNum){
            ...
            //dp
            if(dp[row - 1][col + 1] > maxStep){
                maxStep = dp[row - 1][col + 1];
            }
            else{
        ...
        if( grid[row][col+1] > curNum){
            ...
            //dp
            if(dp[row][col+1] > maxStep){
                maxStep = dp[row][col + 1];
            }
            else{
